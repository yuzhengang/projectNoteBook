...
public class FileUtils {
    ...
    public static String APP_LOG = "FileUtils";//日志文件夹
    /**
     * 检查是否存SD卡
     *
     * @return
    ...
    public static String getRootFilePath( ) {
        ...
        if (isHasSdcard()) {
            ...
            //手机SD卡app私有目录
            path = NoteApplication.mContext.getExternalFilesDir("").getPath() + File.separator;// /storage/emulated/0/Android/data/包命/files
        } else {
            //注意：内部存储没有root的手机不能打开该文件夹的
            //内部存储共有目录
            //path=Environment.getDataDirectory().getAbsolutePath()+File.separator;// /data
            //内部存储app私有目录
            path =NoteApplication.mContext.getFilesDir().getPath() + File.separator;//  /data/user/0/包命/files
        }
        return path;
    }
    ...
    public static String ReadFile2String(String filePath, String charsetName) {
        ...
        Log.e("读取文件是否存在",file.exists()+"    "+file.isFile()+"   "+filePath);
        if (file.isFile() && file.exists()) {
            ...
            try {
                ...
                Log.e("ReadFile2String",e.getLocalizedMessage());
                e.printStackTrace();
            }
        }
        return text;
